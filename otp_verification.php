...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($email) {
            ...
            if ($stmt->execute()) {
                if (sendOTPEmail($email, $user_name, $otp)) {
                    ...
                    $message = "<div class='alert success'>✅ OTP sent successfully to <strong>$email</strong>.</div>";
                    // Reload to show OTP form
                    header("Refresh:1");
                    exit;
                } else {
                    $message = "<div class='alert error'>❌ Failed to send OTP Email.</div>";
                }
            } else {
    ...
    if (isset($_POST['verify_otp'])) {
        ...
        if ($entered_otp && $email) {
            $stmt = $conn->prepare("SELECT v.*, u.id AS user_id 
                                    FROM otp_verification v
                                    JOIN users u ON v.user_id = u.id
                                    WHERE v.email = ? AND v.otp = ?");
            ...
            if ($row = $res->fetch_assoc()) {
                if (strtotime($row['expires_at']) > time()) {
                    // OTP valid — redirect to user's QR code
                    session_unset();
                    header("Location: generatedqr.php?user_id=" . $row['user_id'] . "&verified=1");
                    exit;
                } else {
                    $message = "<div class='alert error'>⏰ OTP expired. Please request a new one.</div>";
                }
            } else {
                $message = "<div class='alert error'>❌ Invalid OTP. Please try again.</div>";
            }
            $stmt->close();
        }
    }
...
<head>
    ...
    <style>
        .otp-container { max-width: 500px; margin: 50px auto; padding: 2rem; background: white; border-radius: 16px;
            box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04); font-family: 'Poppins', sans-serif;}
